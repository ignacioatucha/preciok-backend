...
async def lifespan(app: FastAPI):
    ...
    prices = await database.get_latest_prices()
    if not prices:
        logger.info("🔄 BD vacía — corriendo scraping inicial...")
        await sched.run_scrapers()
    yield
    s.shutdown()
...
async def get_deals(
    ...
    items = await database.get_latest_prices(category=category, barrio=barrio, q=q)
    deals = []
...
async def get_history(
    ...
    history = await database.get_price_history(name, restaurant)
...
@app.get("/api/stats")
async def get_stats():
    all_items = await database.get_latest_prices()
    if not all_items:
